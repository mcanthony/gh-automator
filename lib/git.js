...
exports.createBranch = function(branch) {
    ...
    if (git.status !== 0) {
        currentGitBranch = instance.getCurrentBranch();

        if (branch == currentGitBranch.stdout) {
            logger.log('Cannot create branch ' + branch + ': ' + branch + ' is currently checked out.');
            ...
            for (var i = 0; i < localGitBranchesArray.length; i++) {
                if (branch == localGitBranchesArray[i].substring(2, localGitBranchesArray[i].length)) {
                    logger.log('Cannot create branch ' + branch + ': ' + branch + ' already exists as a local branch.');
    ...
    instance.checkoutBranch(branch);
};
